...
parser.add_argument(
    '--target-metric',
    choices=('params', 'flops', 'hardware'),
    default='params',
    help='stop when the requested parameter, FLOPs, or hardware-cost reduction is reached',
)
parser.add_argument(
    '--cost-source',
    choices=('flops', 'hardware'),
    default='flops',
    help='layer-wise penalty cost; flops preserves original behavior',
)
parser.add_argument(
    '--hardware-profile',
    default='',
    help='JSON generated by profile_layer_hardware.py',
)
parser.add_argument(
    '--hardware-cost-mode',
    choices=('lookup', 'predictor'),
    default='lookup',
...
def configure_search_objective():
    ...
    if args.target_metric == 'hardware' and args.cost_source != 'hardware':
...
def main():
    ...
    if args.cost_source == 'hardware':
        ...
        if hardware_model.get('name') != args.model:
            raise ValueError(
                "Hardware profile model '{}' does not match search model '{}'.".format(
                    hardware_model.get('name'), args.model
                )
            )
        if hardware_model.get('layout') != args.layout:
            raise ValueError(
                "Hardware profile layout '{}' does not match search layout '{}'.".format(
                    hardware_model.get('layout'), args.layout
                )
            )
        candidate_n = [2 ** exponent for exponent in range(int(np.log2(args.M)) + 1)]
        hardware_cost_model.validate_model_layers(
            model.check_N_M().keys(), args.M, candidate_n=candidate_n
        )
        maximum_hardware_reduction = hardware_cost_model.maximum_independent_reduction()
        if (
            args.target_metric == 'hardware'
            and args.target_sparsity > maximum_hardware_reduction + 1e-8
        ):
            raise ValueError(
                "Requested hardware reduction {:.4f} is infeasible for this profile; "
                "the independent per-layer upper bound is {:.4f}. Best scheme: {}".format(
                    args.target_sparsity,
                    maximum_hardware_reduction,
                    hardware_cost_model.best_independent_scheme(),
                )
            )
        if rank == 0:
            print(
                "Hardware cost: {} mode from '{}' on {}".format(
                    args.hardware_cost_mode,
                    args.hardware_profile,
                    hardware_cost_model.profile['hardware'].get('name'),
                )
            )
            print(
                'Independent per-layer upper bound for hardware reduction: {:.4f}'.format(
                    maximum_hardware_reduction
                )
            )
    ...
    decay_normalized = normalize_with_cost(model,include_first=True)
    if args.rank ==0:
        print("{} normalization".format(args.cost_source))
    ...
    if args.rank ==0:
        print("use (ERK * {} + {} * {}) as normalization".format(w1,args.cost_source,w2))
...
def save_searched_scheme(scheme, metrics=None):
    ...
    manifest = {
        ...
        'objective': {
            ...
            'hardware_cost_mode': (
                args.hardware_cost_mode if args.cost_source == 'hardware' else None
            ),
        },
        'hardware_profile': (
            asdict(hardware_cost_model.manifest())
            if hardware_cost_model is not None
            else None
        ),
        ...
        if isinstance(mod, SparseConv) or isinstance(mod, SparseLinear):
            ...
            #     continue
                ...
                if flag_ == True :
                    ...
                    current_hardware_ratio = (
                        check_current_hardware_cost_ratio(net)
                        if hardware_cost_model is not None
                        else None
                    )
                    ...
                    if args.rank == 0:
                        ...
                        if current_hardware_ratio is not None:
                            print('Current hardware cost reduction is {}'.format(current_hardware_ratio))
                        print('Current overall sparsity is {}'.format(current_overall_sparsity))
                        print('Schemes at iterations {} is '.format(num_iters))
                        print(net.check_N_M())

                    cost_normalization = normalize_with_cost(net,include_first=True)
                    ...
                    new_normalized = average_two_normalization(erk_normal,cost_normalization,w1,w2)

                    # new_normalized = flops_normalization

                    if args.rank == 0:
                        print('new {} normalization'.format(args.cost_source))
                        print(cost_normalization)
                        print('new erk normalization')
                        print(erk_normal)
                        print("updating normalization with (erk * {} + {} * {})".format(w1,args.cost_source,w2))
                    ...
                    current_target_value = (
                        ...
                        if args.target_metric == 'params'
                            ...
                            current_flops_ratio
                            if args.target_metric == 'flops'
                            else current_hardware_ratio
                        )
                    )
                        ...
                        if args.rank == 0:
                            ...
                            save_searched_scheme(
                                net.check_N_M(),
                                {
                                    ...
                                    'hardware_cost_reduction': current_hardware_ratio,
...
def normalize_with_cost(net, include_first=True):
    if args.cost_source == 'flops':
        return normalize_with_flops(net, include_first=include_first)
    scheme = net.check_N_M()
    normalized = hardware_cost_model.normalization(scheme)
    if include_first:
        return normalized
    valid = [value for name, value in normalized.items() if 'SparseConv0' not in name]
    maximum = max(valid)
    return {name: value / maximum for name, value in normalized.items()}


def check_current_hardware_cost_ratio(net):
    if hardware_cost_model is None:
        raise RuntimeError('Hardware cost model is not configured.')
    return hardware_cost_model.reduction(net.check_N_M())
